...
export default function AfterHoursConsultationPage() {
  ...
  const getAvailableTimeSlots = () => {
    ...
    if (isWeekend) {
      ...
      for (let hour = startHour; hour <= endHour; hour++) {
        // Add the top of the hour slot (e.g., 9:00, 10:00)
        const topOfHourValue = `${hour.toString().padStart(2, '0')}:00`;
        if (!bookedSlots.includes(topOfHourValue)) {
          const displayHour = hour > 12 ? hour - 12 : hour;
          const ampm = hour >= 12 ? 'PM' : 'AM';
          slots.push({
            value: topOfHourValue,
            label: `${displayHour}:00 ${ampm}`,
          });
        }

        // Add the half-hour slot (e.g., 9:30, 10:30) - but not for the last hour
        if (hour < endHour) {
      ...
      // Weekday: 5:30 PM - 9:00 PM
      const startHour = 17; // 5 PM
      const endHour = 21; // 9 PM

      for (let hour = startHour; hour <= endHour; hour++) {
        // For 5:30 PM, we need to handle the special case
        if (hour === 17) {
          const halfHourValue = '17:30';
          if (!bookedSlots.includes(halfHourValue)) {
            slots.push({
              value: halfHourValue,
              label: '5:30 PM',
          ...
          // Add the top of the hour slot (e.g., 6:00, 7:00)
          const topOfHourValue = `${hour.toString().padStart(2, '0')}:00`;
          if (!bookedSlots.includes(topOfHourValue)) {
            const displayHour = hour > 12 ? hour - 12 : hour;
            const ampm = hour >= 12 ? 'PM' : 'AM';
          ...
          // Add the half-hour slot (e.g., 6:30, 7:30) - but not for the last hour
          if (hour < endHour) {
            const halfHourValue = `${hour.toString().padStart(2, '0')}:30`;
            if (!bookedSlots.includes(halfHourValue)) {
              const displayHour = hour > 12 ? hour - 12 : hour;
  ...
  const handleProceed = async () => {
    ...
    try {
      const res = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({
          ...form,
          consultationType: AFTER_HOURS_OPTION.type,
          consultationName: AFTER_HOURS_OPTION.name,
          consultationPrice: AFTER_HOURS_OPTION.price,
          stripeAmount: AFTER_HOURS_OPTION.stripeAmount,
          consultationDuration: AFTER_HOURS_OPTION.duration,
        }),
      });
      const data = await res.json();
      if (data.url) {
        ...
        sessionStorage.setItem(
          ...
          JSON.stringify({
            ...form,
            selectedType: AFTER_HOURS_OPTION.type,
          })
        );
        window.location.href = data.url;
      } else {
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-16 mb-8">
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              After Hours Legal Consultation
            </h1>
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">
              Get urgent legal advice outside of regular business hours.
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Available: Weekdays 5:30 PM - 9:00 PM, Weekends 9:00 AM -
                      12:00 PM
                    </p>
                  </div>
                  <div className="flex-1">
                ...
                <div className="border rounded-xl p-6 bg-blue-50 border-blue-200">
                  <div className="flex justify-between items-start mb-2">
                    <div className="font-semibold text-lg text-gray-900 flex items-center gap-2">
                      {AFTER_HOURS_OPTION.name}
                      <CheckCircle2 className="w-5 h-5 text-blue-600" />
                    </div>
                    <div className="text-blue-600 font-bold">
                      AUD ${AFTER_HOURS_OPTION.price}
                    </div>
                  </div>
                  <div className="text-gray-600 mb-2 flex items-center gap-2">
                    <Clock className="w-4 h-4" />
                    {AFTER_HOURS_OPTION.duration}
                  </div>
                  <div className="text-gray-500">
                    {AFTER_HOURS_OPTION.description}
                  </div>
                </div>
              </div>
            </div>
