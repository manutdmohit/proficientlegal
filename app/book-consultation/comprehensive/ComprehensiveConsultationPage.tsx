...
export default function ComprehensiveConsultationPage() {
  ...
  const getAvailableTimeSlots = () => {
    ...
    // Regular business hours: 9:00 AM - 5:00 PM (weekdays only)
    if (!isWeekend) {
  ...
  const handleProceed = async () => {
    ...
    try {
      const res = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({
          ...form,
          consultationType: COMPREHENSIVE_OPTION.type,
          consultationName: COMPREHENSIVE_OPTION.name,
          consultationPrice: COMPREHENSIVE_OPTION.price,
          stripeAmount: COMPREHENSIVE_OPTION.stripeAmount,
          consultationDuration: COMPREHENSIVE_OPTION.duration,
        }),
      });
      const data = await res.json();
      if (data.url) {
        ...
        sessionStorage.setItem(
          ...
          JSON.stringify({
            ...form,
            selectedType: COMPREHENSIVE_OPTION.type,
          })
        );
        window.location.href = data.url;
      } else {
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-16 mb-8">
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">
              Get a full hour with a senior lawyer to discuss your matter in
              detail, review documents, and receive tailored legal advice.
            </p>
          </div>
        </div>
                ...
                <div className="border rounded-xl p-6 bg-blue-50 border-blue-200">
                  <div className="flex justify-between items-start mb-2">
                    <div className="font-semibold text-lg text-gray-900 flex items-center gap-2">
                      {COMPREHENSIVE_OPTION.name}
                      <CheckCircle2 className="w-5 h-5 text-blue-600" />
                    </div>
                    <div className="text-blue-600 font-bold">
                      AUD ${COMPREHENSIVE_OPTION.price}
                    </div>
                  </div>
                  <div className="text-gray-600 mb-2 flex items-center gap-2">
                    <Clock className="w-4 h-4" />
                    {COMPREHENSIVE_OPTION.duration}
                  </div>
                  <div className="text-gray-500">
                    {COMPREHENSIVE_OPTION.description}
                  </div>
                </div>
              </div>
            </div>
