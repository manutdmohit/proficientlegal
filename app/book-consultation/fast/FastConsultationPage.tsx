...
export default function FastConsultationPage() {
  ...
  const handleProceed = async () => {
    ...
    try {
      const res = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({
          ...form,
          consultationType: FAST_OPTION.type,
          consultationName: FAST_OPTION.name,
          consultationPrice: FAST_OPTION.price,
          stripeAmount: FAST_OPTION.stripeAmount,
          consultationDuration: FAST_OPTION.duration,
        }),
      });
      const data = await res.json();
      if (data.url) {
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-16 mb-8">
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <p className="text-xl text-blue-100 max-w-2xl mx-auto">
              Get quick 10-minute advice for urgent legal questions that need
                ...
                <div className="border rounded-xl p-6 bg-blue-50 border-blue-200">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="text-blue-600 font-bold">
                      AUD ${FAST_OPTION.price}
                    </div>
                  </div>
                  <div className="text-gray-600 mb-2 flex items-center gap-2">
                    <Clock className="w-4 h-4" />
                    {FAST_OPTION.duration}
                  </div>
                  <div className="text-gray-500">{FAST_OPTION.description}</div>
                </div>
              </div>
            </div>
